...
export async function getHomePageSnapshot() {
  ...
  return getDashboardSnapshot({
    ...
    learnerRepository: createInMemoryLearnerRepository({
      ...
      setAggregates: buildPreviewSetAggregates(setDefinitions),
    }),
    now: new Date(),
  });
}

function buildPreviewSetAggregates(setDefinitions: ContentSetDefinition[]): SetAggregate[] {
...
function buildPreviewSetAggregate(
  ...
  if (index === 2) {
    return {
      setId: setDefinition.id,
      totalItems: setDefinition.totalItems,
      itemsSeen: Math.min(Math.max(18, Math.floor(setDefinition.totalItems * 0.35)), setDefinition.totalItems),
