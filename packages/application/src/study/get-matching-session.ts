...
export interface MatchingSessionSnapshot {
...
export interface GetMatchingSessionDependencies {
  contentRepository: ContentRepository;
  selection: SessionSelection;
  limit?: number;
}

export async function getMatchingSession({
  contentRepository,
  selection,
  limit = 8,
}: GetMatchingSessionDependencies): Promise<MatchingSessionSnapshot | null> {
  ...
  const eligibleEntries = selectedEntries.filter((entry) => entry.modeEligibility.matchingSafe);
