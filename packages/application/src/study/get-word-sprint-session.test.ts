...
import { getWordSprintSession } from './get-word-sprint-session';
...
describe('getWordSprintSession', () => {
  it('uses the shared typing-safe session shape with a larger default limit', async () => {
    const snapshot = await getWordSprintSession({
      contentRepository,
      selection: {
        themeIds: [],
        categoryIds: [],
    ...
    expect(snapshot?.eligibleCount).toBe(2);
